...
namespace Town
{
		...
		, m_MenuContent( Vec2i{96,32} )
	{
		...
		{//メニュー
			...
			m_MenuContent
				.Add( { Icon( BMP( Icon16::Inn ), 16+6 ), Text( L"宿" ) } )
				.Add( { Icon( BMP( Icon16::Pub ), 16+6 ), Text( L"酒場" ) } )
				.Add( { Icon( BMP( Icon16::Shop ), 16+6 ), Text( L"店" ) } )
				.Add( { Icon( BMP( Icon16::Maze ), 16+6 ), Text( L"迷宮" ) } )
				.CursorPos( 0 );
	...
	Flags<GUIResult> TownScene::TownCenter_UI::Update( const IController &Controller )
	{
		...
		case HandleInputResult::Selected:
			{
				m_ImgPainter.SetImg( &ResManage::BMP( ResManage::TownBkgnd::TownCenter ) );	//このタイミングで背景画像をリセット
